    ...
    for y in range(board_height):
        for x in range(board_width):
            ...
            if cell and cell['id'] in slate_db:
                slate_id = cell['id']
                rotation = str(cell.get('rotation', 0)) # JSON 키는 문자열이므로 변환
                ...
                for buff in buffcoords:
                    # Y축 보정 및 실제 좌표 계산
                    eff_x, eff_y = x + buff['x'], y - buff['y']
                    
                    if 0 <= eff_y < board_height and 0 <= eff_x < board_width:
                        # effect가 "none"이 아닐 경우, 나중에 특별한 로직 추가 가능
                        if buff.get('effect') != "none":
                            pass # 예: 'limitUnlock' 효과 처리
                        effect_map[eff_y][eff_x] += buff.get('level', 0)
        ...
        for x in range(board_width):
            ...
            if cell and cell['id'] in artifact_db:
                ...
                if artifact_level >= 0:
                    ...
                    total_score += (artifact_level * weight)
                    
    return total_score

# --- 3. 핵심 함수: 최적 배치 탐색기 ---

def find_optimal_placement(item_ids_to_place, board_width, board_height, iterations=10000):
    """반복 개선 방식으로 최적의 배치를 찾습니다."""
    
    items = [{'id': item_id, 'rotation': 0} for item_id in item_ids_to_place]
    ...
    for _ in range(iterations):
        ...
        temp_coords = list(placed_items_coords)
        
        action = random.choice([0, 0, 0, 1]) 
        
        if action == 0 and len(temp_coords) >= 2:
            (y1, x1), (y2, x2) = random.sample(temp_coords, 2)
            temp_board[y1][x1], temp_board[y2][x2] = temp_board[y2][x2], temp_board[y1][x1]
        
        elif action == 1 and temp_coords:
            y, x = random.choice(temp_coords)
            cell = temp_board[y][x]
...
if __name__ == '__main__':
    my_items = ["aritifact_115", "aritifact_118", "slate_2", "slate_7"]
    
    inventory_width = 5
    inventory_height = 5
    
    print(f"'{', '.join(my_items)}' 아이템으로 최적 배치를 탐색합니다...")
    
    optimal_board, best_score = find_optimal_placement(my_items, inventory_width, inventory_height, iterations=20000)
    
    print("\n[ 최적 배치 결과 ]")
    print(f"최고 점수: {best_score:.2f}")
    print("-" * (inventory_width * 22))
    for row in optimal_board:
        ...
        for cell in row:
            if cell:
                ...
                rotation = f"({cell.get('rotation', 0)}°)" if cell['id'] in slate_db and slate_db[cell['id']].get('rotatable') else ""
                display_row.append(f"{item_name}{rotation:<20}")
            else:
                display_row.append(f"{'---':<20}")
