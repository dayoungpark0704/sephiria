                ...
                if condition == 'top_row' and y != 0: is_violated = True
                    ...
                    if x < board_width - 1:
                        ...
                        if right_cell and right_cell['id'] in artifact_db:
                            ...
                            if "마법서" in right_artifact.get("tags", []):
                                is_violated = False
                elif condition == 'requires_surrounding_planets':
                    is_violated = False 
                    surrounding_coords = [
                    ...
                    for dx, dy in surrounding_coords:
                        ...
                        if not (0 <= nx < board_width and 0 <= ny < board_height):
                            is_violated = True; break
                        neighbor_cell = board[ny][nx]
                        if not neighbor_cell or neighbor_cell['id'] not in artifact_db:
                            is_violated = True; break
                        neighbor_artifact = artifact_db[neighbor_cell['id']]
                        if "행성" not in neighbor_artifact.get("tags", []):
                            is_violated = True; break
...
if __name__ == '__main__':
    my_items_data = [
        {"id": "artifact_60", "priority": 2.0, "upgrade": 0},      # 거대 망원경
        {"id": "artifact_12", "priority": 1.0, "upgrade": 0},      # 푸른 행성
        {"id": "artifact_57", "priority": 1.0, "upgrade": 0},      # 노란 행성
        {"id": "artifact_58", "priority": 1.0, "upgrade": 0},      # 붉은 행성
        {"id": "artifact_59", "priority": 1.0, "upgrade": 0},      # 하늘색 행성
        {"id": "artifact_184", "priority": 1.0, "upgrade": 0},     # 하얀 행성
        {"id": "slate_7", "priority": 1.0, "upgrade": 0}
    ]
