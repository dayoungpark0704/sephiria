...
def calculate_score(board, board_width, board_height):
    ...
    # 2-1. 석판 효과 맵 생성 (레벨 효과와 특수 효과 분리)
    ...
    for y in range(board_height):
        for x in range(board_width):
            ...
            if cell and cell['id'] in artifact_db:
                artifact = artifact_db[cell['id']]
                upgrade_level = cell.get('upgrade', 0)
                user_priority = cell.get('priority', 1.0)
                
                additional_level = upgrade_level * 1 # 강화 레벨당 +1 가정
                        ...
                        if condition_type == 'top_row' and y != 0: is_violated = True
                            ...
                            if x < board_width - 1 and board[y][x+1] and board[y][x+1]['id'] in artifact_db:
                                if "마법서" in artifact_db[board[y][x+1]['id']].get("tags", []):
                                    is_violated = False
                        elif condition_type == 'requires_surrounding_planets':
                            is_violated = False
                            ...
                            for dx, dy in surrounding_coords:
                                ...
                                if 0 <= nx < board_width and 0 <= ny < board_height:
                                    ...
                                    if neighbor_cell is not None:
                                        if neighbor_cell['id'] not in artifact_db or \
                                           "행성" not in artifact_db[neighbor_cell['id']].get("tags", []):
...
if __name__ == '__main__':
    my_items_data = [
        {"id": "artifact_60", "priority": 2.0, "upgrade": 0},
        {"id": "artifact_12", "priority": 1.0, "upgrade": 0},
        {"id": "artifact_57", "priority": 1.0, "upgrade": 0},
        {"id": "artifact_58", "priority": 1.0, "upgrade": 0}
    ]
    
    inventory_width, inventory_height = 5, 5
