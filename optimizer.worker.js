...
function isSlotAvailable(slotIndex, condition, currentGrid, calculateSlotsFunc) {
    ...
    switch (condition) {
        ...
        case "최하단": const slotBelowIndex = slotIndex + 6; return slotBelowIndex >= calculateSlotsFunc();
        case "가장자리": return row === 0 || row === totalRows - 1 || col === 0 || col === totalCols - 1;
        case "안쪽": return row > 0 && row < totalRows - 1 && col > 0 && col < totalCols - 1;
        case "양쪽칸이 공백":
            const leftSlotIndex = slotIndex - 1; const rightSlotIndex = slotIndex + 1;
            const isLeftInBounds = (col > 0); const isRightInBounds = (col < totalCols - 1);
...
function calculateSlotBuffs(currentGridItemsState, maxSlotsConst, calculateSlotsFunc) {
    ...
    for (let slotIndex = 0; slotIndex < currentGridItemsState.length; slotIndex++) {
        ...
        if (itemInstance && itemInstance.isSlate) {
            const baseRow = Math.floor(slotIndex / 6); const baseCol = slotIndex % 6;
            const rotation = itemInstance.rotation || 0;
            itemInstance.buffcoords.forEach(buff => {
                let transformedX = buff.x || 0; let transformedY = buff.y || 0;
                ...
                const targetRow = baseRow + transformedY; const targetCol = baseCol + transformedX;
                const targetIndex = targetRow * 6 + targetCol;
                const currentActiveSlotsCount = calculateSlotsFunc();
                if (targetIndex >= 0 && targetIndex < currentActiveSlotsCount && targetRow >= 0 && targetRow < Math.ceil(currentActiveSlotsCount / 6) && targetCol >= 0 && targetCol < 6) {
                    let buffValue = 0;
                    let valueSource = buff.value !== undefined ? buff.value : buff.v;
                    if (buff.type) {
                        if (buff.type === 'edge' && isSlotAvailable(targetIndex, '가장자리', currentGridItemsState, calculateSlotsFunc)) buffValue = !isNaN(parseInt(valueSource)) ? parseInt(valueSource) : 0;
                        else if (buff.type === 'downside' && isSlotAvailable(targetIndex, '최하단', currentGridItemsState, calculateSlotsFunc)) buffValue = !isNaN(parseInt(valueSource)) ? parseInt(valueSource) : 0;
                    } else if (valueSource !== undefined) {
                        if (valueSource === "limitUnlock") buffValue = 1;
                        else if (!isNaN(parseInt(valueSource))) buffValue = parseInt(valueSource);
                    }
    ...
    if (maxScore > -1) {
        ...
        if (remainingMaxScore === undefined) {
            ...
            for (let i = itemIndex; i < allItemsToPlace.length; i++) {
                if (!allItemsToPlace[i].isSlate) remainingMaxScore += allItemsToPlace[i].maxUpgrade;
            }
            maxPossibleScoreCache.set(itemIndex, remainingMaxScore);
        }
        if (currentScore + remainingMaxScore <= maxScore) return;
    }
