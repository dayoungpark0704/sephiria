document.addEventListener('DOMContentLoaded', async () => {
                    ...
                    <div class="item-info">
                        ...
                        <div class="item-controls-group">
                            ...
                            <div class="item-controls">
                                ...
                                <span>${itemState.priority.toFixed(1)}</span>
                                <button class="control-btn priority-btn" data-change="0.1">+</button>
                            </div>
                        </div>
                    </div>
    ...
    inventoryGrid.addEventListener('drop', (e) => {
        ...
        if (draggedItem && slot) {
            ...
            ownedItems[itemId]--;
            inventoryState[slotId] = { id: itemId, uniqueId: nextUniqueId++, priority: 1.0, upgrade: 0 };
    ...
    selectedArtifactsList.addEventListener('click', (e) => {
        ...
        if (target.classList.contains('enchant-btn')) {
            itemState.upgrade = Math.max(0, Math.min(dbItem.maxUpgrade, itemState.upgrade + parseInt(change)));
        } else if (target.classList.contains('priority-btn')) {
            itemState.priority = Math.max(0.1, parseFloat((itemState.priority + change).toFixed(1)));
        }
