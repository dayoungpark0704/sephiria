document.addEventListener('DOMContentLoaded', async () => {
    ...
    function updateSlots(count) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (oldState[i]) {
                ...
                renderSlot(slot, oldState[i]); // renderSlot 함수 사용
            }
    ...
    itemList.addEventListener('click', (e) => {
        ...
        if (!target.classList.contains('stack-btn')) return;

        const card = e.target.closest('.item-card');
        const itemId = card.dataset.itemId;
        const change = parseInt(target.dataset.change);
        
        const oldCount = ownedItems[itemId] || 0;
    ...
    document.getElementById('optimize-btn').addEventListener('click', async () => {
        ...
        try {
            ...
            inventoryState = result.board.flat(); // 2D 배열을 1D로 변환
            updateSlots(inventoryState.length);
            updateSelectedItems();
