...
function isSlotAvailable(slotIndex, condition, currentGrid, calculateSlotsFunc) {
    ...
    const totalRows = Math.ceil(calculateSlotsFunc() / 6); // calculateSlotsFunc 사용
    const totalCols = 6;
 ...
 */
function findSolution(itemIndex, currentGridState, allItemsToPlace, totalActiveSlots, isSlotAvailableFunc) {
    ...
    for (let slotCandidateIndex = 0; slotCandidateIndex < totalActiveSlots; slotCandidateIndex++) {
        if (currentGridState[slotCandidateIndex] === null &&
            isSlotAvailableFunc(slotCandidateIndex, itemCondition, currentGridState, calculateSlots)) { // calculateSlots를 isSlotAvailableFunc에 전달
            ...
            if (currentItemInstance.id.startsWith('slate_') && currentItemInstance.rotatable) {
                ...
                for (let rot = 0; rot < 360; rot += 90) {
                    ...
                    if (findSolution(itemIndex + 1, currentGridState, allItemsToPlace, totalActiveSlots, isSlotAvailableFunc)) {
                        return true;
                    }
                }
